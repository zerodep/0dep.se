...
- **No frameworks, no bundler** for the pages themselves. Plain HTML and CSS. The renderer in `src/build.js` is pure string concatenation with explicit HTML escaping. Exception: the runner pages (`/run/`, `/dmn/`, `/tools/`, `/toc/`) ship ESM bundles built by esbuild from `src/runner/` — all devDependencies, bundled at build time (bpmn-elements needs `>=18.1.0` — browser-safe timers, the `Expressions` export, step-mode transaction/compensation completion, the `accepts` array on postponed content, cancellable conditional events, and the `assignOutput` setting that merges output of plain elements @0dep/bpmn-extensions skips; @0dep/bpmn-extensions `>=0.0.6` for its cron-capable `TimerEventDefinition`, registered in the runner's type resolver — it peer-depends on `croner`, declared as a devDependency so the bundle picks it up). The zeebe and camunda schemas conflict in moddle (same `modelerTemplate` extension property), so the runner registers one per parse based on the xmlns in the source. The `/tools/` page keeps its attempt history in `localStorage` under `0dep-tools-history` (50 entries, deduped by tool+input+options). The `/toc/` page runs @0dep/toc on dropped or pasted markdown (pure logic in `src/runner/toc-lib.js`, DOM in `toc-app.js`); the same package keeps this repo's README toc fresh via `npm run toc` (also `pretest`). Each runner page has its own service worker (`run-`/`dmn-`/`tools-`/`toc-` cache prefixes): network-first with the cache as offline fallback, precached past the HTTP cache (`cache: 'reload'`), versioned by a hash of every precached file and written last in the build.
- **Single source of truth.** Project list lives in `data/projects.json`. Add or remove packages there; tests and the build script consume it. Each project's `runtimeDeps` must mirror the package's published `dependencies` and `peerDeps` its required `peerDependencies` (optional peers, per `peerDependenciesMeta`, are left out) — a test cross-checks both against whatever is installed in `node_modules`, so listed packages are added as devDependencies where practical. The zero-deps ring on a card is only rendered when both lists are empty.
...
- **zerodep packages** (under `github.com/zerodep`): `@0dep/piso`, `@0dep/pino-applicationinsights`, `ocrgenerator`, `texample`, `@0dep/toc` (has its own `/toc/` page), `amqp-emulator` (builds on `smqp`).
